...
```{r cleaning_data, echo=FALSE, message = FALSE, results='asis'}
require(mosaic)
require(xtable)
require(dplyr)
...
The following chapters outline individual analyses done on these data.   
   1. Missing Data and Multiple Imputation by Joshua Freeman   
   2. Comparisons of Automatic Model Selection Methods by Coco Kusiak   
   3. Multi-Level Modeling with Random Effects by Luke Toomey    
...
##Complete Case Model##
fit.cc<-glm(formula=mental~as.factor(state) + sleep + male + as.factor(employ) + as.factor(actlimit) + as.factor(metro) + exer30 + as.factor(race) + as.factor(income) + as.factor(smoker) + binge, family="binomial", data=mhfp)
...
mi.fit=glm.mids(mental~as.factor(state) + sleep + male + as.factor(employ) + as.factor(actlimit) + as.factor(metro) + exer30 + as.factor(race) + as.factor(income) + as.factor(smoker) + binge, family="binomial", data=imputedat)
summary(mi.fit)
...
##Pooled Imputed Model##
mi.fit50=glm.mids(mental~as.factor(state) + sleep + male + as.factor(employ) + as.factor(actlimit) + as.factor(metro) + exer30 + as.factor(race) + as.factor(income) + as.factor(smoker) + binge, family="binomial", data=imputedat50)
...
```{r}
tab1 <- as.data.frame(summval)
tab1 <- mutate(tab1, id = rownames(tab1))
tab1 <- filter(tab1, id != "actlimit2")
tab2 <- as.data.frame(summary(comb.test)[,c("est","se","fmi")])
tab2 <- mutate(tab2, id = rownames(tab3))
tab3 <- as.data.frame(summary(comb.test50)[,c("est","se","fmi")])
tab3 <- mutate(tab3, id = rownames(tab4))

combined1 <- left_join(tab1, tab2, by = "id")
combined2 <- left_join(combined1,tab3, by = "id")
rownames(combined) <- combined$id
combined <- subset(combined, select = - id)
colnames(combined) <- c("CC.Estimates", "CC.SE", "MI20.Estimates", "MI20.SE", "MI50.Estimates", "MI50.SE")
print(xtable(combined, caption = "Title", caption.place = 'top', digits = 5))
```
...
##Truth Model##
fit.tval<-glm(formula=mental~as.factor(state) + sleep + male + as.factor(employ) + as.factor(actlimit) + as.factor(metro) + exer30 + as.factor(race) + as.factor(income) + as.factor(smoker) + binge, family="binomial", data=val2)
summtruth<-summary(fit.tval)$coef[,c("Estimate","Std. Error")]
#summtruth<-summtruth[-c(2:19,22:28,34:37), ]
print(xtable(summtruth))

##Complete Case Model##
fit.ccval<-glm(formula=mental~as.factor(state) + sleep + male + as.factor(employ) + as.factor(actlimit) + as.factor(metro) + exer30 + as.factor(race) + as.factor(income) + as.factor(smoker) + binge, family="binomial", data=randodat)
summval<-summary(fit.cc)$coef[,c("Estimate","Std. Error")]
summval<-summval[-c(2:19,22:28,34:37), ]
print(xtable(summary(fit.cc)$coef[,c("Estimate","Std. Error")]))
...
##Pooled Imputed Model##
mi.fitval=glm.mids(mental~as.factor(state) + sleep + male + as.factor(employ) + as.factor(actlimit) + as.factor(metro) + exer30 + as.factor(race) + as.factor(income) + as.factor(smoker) + binge, family="binomial", data=imputedatval)
head(summary(mi.fitval))
comb.testval = pool(mi.fitval)
##Summary Table##
print(xtable(summary(comb.testval)[,c("est","se","fmi")], caption = "This is the title", caption.placement = 'top'))
...
##Pooled Imputed Model##
mi.fit50val=glm.mids(mental~as.factor(state) + sleep + male + as.factor(employ) + as.factor(actlimit) + as.factor(metro) + exer30 + as.factor(race) + as.factor(income) + as.factor(smoker) + binge, family="binomial", data=imputedat50val)
...
tab1 <- mutate(tab1, id = rownames(tab1))
tab1 <- filter(tab1, id != "actlimit2")
tab2 <- as.data.frame(summtruth)
tab2 <- mutate(tab2, id = rownames(tab2))
tab3 <- as.data.frame(summary(comb.testval)[,c("est","se","fmi")])
tab3 <- mutate(tab3, id = rownames(tab3))
tab4 <- as.data.frame(summary(comb.test50val)[,c("est","se","fmi")])
tab4 <- mutate(tab4, id = rownames(tab4))

combined1 <- left_join(tab1, tab2, by = "id")
combined2 <- left_join(combined1,tab3, by = "id")
combined <- left_join(combined2, tab4, by = "id")
rownames(combined) <- combined$id
combined <- subset(combined, select = - id)
colnames(combined) <- c("Truth.Estimates", "Truth.SE", "CC.Estimates", "CC.SE", "MI20.Estimates", "MI20.SE", "MI50.Estimates", "MI50.SE")
print(xtable(combined, caption = "Title", caption.place = 'top', digits = 5))
```
...
```{r cocos_packages, include=FALSE, message=FALSE}
require(glmnet)
require(MASS)
require(pROC)
require(mosaic)
...
```{r fitting_algorithms, eval=FALSE, echo = FALSE}
load("completeimputed50.Rda")
ment <- compdat50 
...
```{r plotting, echo=FALSE, fig.height=4}
load("coco.Rda")
...
```{r coefficients_table, echo=FALSE, results ='asis'}
...
```{r cross_validation, eval = FALSE, echo=FALSE}
...
```{r comparing_cv, echo = FALSE, results='asis'}
...
```{r comparing_sims, echo = FALSE, results='asis'}
...
The table below shows a few keys predictors for sleep.  

```{r key_variables, results='asis', echo =FALSE}
associations <- data.frame(var = c("Binge Drinking", "Exercise", "Male", "Sleep", "Transgender", "State = Puerto Rico", "State = Oregon"), 
                           direction = c("Positive", "Negative", "Negative", "Negative", "Positive", "Negative", "Positive"),
                           ro = c("+53%", "-8%", "-40%", "-11%", "+22%", "-45%", "+30%"),
                           od = c(1.532, 0.9185, 0.6039, 0.8949, 1.2229, 0.5504, 1.3038),
                           ci = c("(1.488), 1.576)", "(0.897, 0.939)", "(0.592, 0.617)", 
                                        "(0.889, 0.901)", "(1.04, 1.43)", "(0.549, 0.651)", "(1.21, 1.39)")
                           
                           )
colnames(associations) <- c("Variable", "Direction of Association", "% Relative Odds", "Odds Ratio", "95% CI")
print(xtable(associations, caption="Key Variables"), caption.placement = 'top')
```  

Binge drinking is positively associated with having poor mental health. Those who binge drink have a 53% higher odds of reporting depression or anxiety in the past 30 days. Males have a 40% lower odds of reporting poor mental health than females. Another interesting finding in that living in Puerto Rico is negatively associated with having poor mental health while living in Oregon is positively associated. The relationship between binge drinking and gender are interesting and provide insight into the associaiton between prevalence of poor mental health and other covariates.  Further research should be conducted to examine how binge drinking and mental health are related temporally given the BRFSS only captures a cross-section of data.  Binge drinking could therefore act as a copacetic for mental health if mental health truly precedes binge drinking or binge drinking is a driver of poor mental health.
