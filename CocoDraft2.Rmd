...
simulated <- subset(simulated, select = - c(mental, index))

auc10_lasso <- c()
auc10_ridge <- c()
auc10_aic <- c()
for (i in 1:10){
  data <- sample(simulated, size = 1000)
  #index <- data$index
  data <- mutate(data, 
                 mental = as.factor(ifelse(predict(model.AIC, newx = data, type = "response")[orig.id]< .5, yes = 0, no = 1)))
